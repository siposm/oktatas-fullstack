# Gyakorló feladatok (backend + frontend)
...
A szerveren perzisztens adattárolásra nincs szükség (de gyakorolni azt is lehet, ez esetben egészítse ki a feladatot azzal pluszban), kontrollerre és modelre viszont igen - ezeket a tanultaknak megfelelően hozza létre. A szerver feladata, hogy API-n keresztül megkap valamilyen adatot a klienstől, azt feldolgozva, számításokat végezve pedig válaszol a kliensnek. Ezt megkapva a kliens feladata, hogy a kapott választ igényes formában, szép kinézettel megjelenítse.

A javasolt mappaszerkezet a következő. A gyökér mappában két mappát kell létrehozni `BACKEND` és `FRONTEND` néven, amelyekbe majd a szerveroldali és kliensoldali kódok kerülnek.
...
A programot lokális verziókövetéssel javasolt kezelni a tanultaknak megfelelően. Javasolt minimum 20 commit-ot készíteni feladatonként. A commit-oknak követniük kell a "commit small, commit often" elvet, illetve törekedni kell az értelmes commit-ok léterhozására, beszédes üzenetekkel.
